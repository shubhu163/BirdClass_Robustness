...
DATA_DIR = '/nfs/hpc/share/joshishu/DL_Project/BirdClass_Robustness/Adversarial_Wide/Mixed_version_Adv_Images'
...
def train_loop(model: torch.nn.Module,
  ...
  for epoch in tqdm(range(epochs)):
    ...
    if final_valid_loss < best_val_loss:
        ...
        counter=0
        torch.save(model.state_dict(), "WideResnet_ADV_Shuffled_14.pth")
    else:
        counter += 1
        if counter >= patience:
            print("Stopping...")
...
# optimizer = torch.optim.Adam(params=model.parameters(), lr=0.0001, weight_decay = 5e-3)
optimizer = torch.optim.Adam(params=model.parameters(), lr=0.0001, betas=(0.9, 0.999), eps=1e-8, weight_decay=5e-4)
